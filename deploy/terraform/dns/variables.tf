...
variable "project_id" {
  description = <<-EOT
    GCP project that owns the managed zone. Grit Chat is a personal-tier
    product, so this belongs in a personal-tier project rather than in an
    organisation project that belongs to a different owner.
  EOT
  type        = string
  nullable    = false
}
