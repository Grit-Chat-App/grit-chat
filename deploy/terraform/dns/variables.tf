...
variable "firebase_dns_records" {
  description = <<-EOT
    Complete Firebase Hosting DNS RRsets, grouped by normalized
    "<name>|<type>" key. This is generated by deploy/terraform/hosting after
    Firebase creates its CustomDomain associations, then supplied to this root
    by CI from private Terraform state.

    It MUST NOT be copied into a public tfvars file, PR body, or repository
    variable. Firebase generates ownership and certificate records after the
    associations exist. `wait_dns_verification = false` makes those values
    available before GoDaddy delegates to Cloud DNS; the second DNS apply
    materializes every generated record before the manual handoff.

    The empty default is intentionally fail-closed. `web_ready` remains false
    until CI supplies the generated set, even though static Firebase A records
    are documented. One source of truth prevents a later Firebase change from
    leaving a stale literal A or TXT record in the zone.
  EOT
  type = map(object({
    name    = string
    type    = string
